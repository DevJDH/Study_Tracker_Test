...
def advanced_function(func):
    try:
        if func in {"sin", "cos", "tan"}:
            ...
            if use_degrees:
                value = radians(value)  # Convert to radians if in degrees mode
            result = eval(f"{func}({value})")
        else:
            result = eval(f"{func}({screen.get()})")
        screen.set(result)
    except Exception as e:
        screen.set("Error")
...
# Toggle mode between degrees and radians
def toggle_mode():
    global use_degrees
...
# Input screen
screen = tk.StringVar()
entry = tk.Entry(root, textvar=screen, font="lucida 20 bold", bd=10, relief=tk.SUNKEN, justify="right")
...
for i, row in enumerate(buttons):
    for j, btn_text in enumerate(row):
        if btn_text in {"sin", "cos", "tan", "log", "ln", "sqrt"}:
            ...
            btn = tk.Button(
                button_frame, text=btn_text, font="lucida 15 bold", padx=20, pady=15,
                command=lambda const=btn_text: screen.set(screen.get() + str(pi if const == "π" else e))
            )
        else:
            btn = tk.Button(
                button_frame, text=btn_text, font="lucida 15 bold", padx=20, pady=15
